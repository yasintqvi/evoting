...
<form action="{{ route('voting.store', [$company->slug, $election->id]) }}" method="post">
    @csrf
    <div class="row">
        ...
        <div class="col-xl-8 col-lg-12">
                                ...
                                <div class="card-body text-center">
                                    ...
                                    <div class="mt-3 d-flex gap-2 justify-content-center">
                                        ...
                                        <div class="form-check form-checkbox-secondary mb-2">
                                            <input name="director_candidates[{{$candidate->id}}]" @checked(old("inspector_candidates.{{$candidate->id}}")) value="{{ $participant->total_stock }}" type="checkbox" class="form-check-input" id="candidate-director-{{$candidate->id}}">
                                            <label class="form-check-label" for="candidate-director-{{$candidate->id}}">انتخاب کردن</label>
                                        </div>
                                        @else
                                        <div class="mt-2">
                                            <label for="director-candidate-range-{{$candidate->id}}" class="form-label">مقدار سهم:</label>
                                            <input type="range"
                                                name="director_candidates[{{$candidate->id}}]"
                                                id="director-candidate-range-{{$candidate->id}}"
                                                class="form-range"
                                                min="0"
                                                max="{{ $participant->total_stock }}"
                                                value="0"
                                                oninput="updateRangeValue('director-candidate-range-{{$candidate->id}}', 'director-candidate-value-{{$candidate->id}}')">
                                            <div class="d-flex justify-content-center">
                                                <h4 id="director-candidate-value-{{$candidate->id}}">0</h4><span>&nbsp; &nbsp; سهم </span>
                                            </div>
                                        </div>
                                        @endif
                                    </div>
            ...
            <div class="card">
                <div class="card-header border-bottom border-dashed">
                    <h4 class="card-title mb-0">انتخاب کاندیدای بازرس</h4>
                </div>
                <div class="card-body">
                    <div class="row">
                        @foreach ($election->candidates->where('candidate_type', App\Enums\CandidateType::INSPECTOR) as $candidate)
                        <div class="col-xl-3 col-lg-6">
                            <div class="card">
                                <div class="card-body text-center">
                                    <div class="row justify-content-between mb-3">
                                    ...
                                    <div class="mt-3 d-flex gap-2 justify-content-center">
                                        @if (in_array($election->type, [App\Enums\ElectionType::PUBLIC_JOINT,App\Enums\ElectionType::PRIVATE_JOINT]))
                                        <div class="form-check form-checkbox-secondary mb-2">
                                            <input name="inspector_candidates[{{$candidate->id}}]" @checked(old("inspector_candidates.{{$candidate->id}}")) value="{{ $participant->total_stock }}" type="checkbox" class="form-check-input" id="director-candidate-{{ $candidate->id }}">
                                            <label class="form-check-label" for="director-candidate-{{ $candidate->id }}">انتخاب کردن</label>
                                        </div>
                                        @else
                                        <div class="mt-2">
                                            <label for="inspector-candidates-range-{{$candidate->id}}" class="form-label">مقدار سهم:</label>
                                            <input type="range"
                                                name="inspector_candidates[{{$candidate->id}}]"
                                                id="inspector-candidate-range-{{$candidate->id}}"
                                                class="form-range"
                                                min="0"
                                                max="{{ $participant->total_stock }}"
                                                value="0"
                                                oninput="updateRangeValue('inspector-candidate-range-{{$candidate->id}}', 'inspector-candidate-value-{{$candidate->id}}')">
                                            <div class="d-flex justify-content-center">
                                                <h4 id="inspector-candidate-value-{{$candidate->id}}">0</h4><span>&nbsp; &nbsp; سهم </span>
                                            </div>
                                        </div>
                                        @endif
                                    </div>
        ...
        function enforceRangeLimit(ranges, max) {
            ranges.forEach(range => {
                range.addEventListener('input', function() {
                    const activeRanges = Array.from(ranges).filter(r => parseInt(r.value) > 0).length;
