                    ...
                    <div class="row">
                        <div class="mb-3">
                            ...
                            <input type="text" class="form-control" name="title" id="title"
                                placeholder="عنوان را وارد کنید" value="{{ old('title', $group->title) }}" required="">
                            @error('title')
                            <span class="text-danger font-weight-bold">{{ $message }}</span>
                            @enderror
                        </div>

                        <h6 class="fs-13">انتخاب نوع گروه</h6>
                        <div class="mt-2">
                            <div class="form-check form-check-inline d-inline-block me-3">
                                <input type="radio" id="customRadio3" name="type"
                                    value="{{ App\Enums\GroupType::COOPERTAIVE->value }}" class="form-check-input"
                                    {{ $group->type == App\Enums\GroupType::COOPERTAIVE->value ? 'checked' : '' }}>
                                <label class="form-check-label" for="customRadio3">نوع گروه تعاونی</label>
                            </div>
                            <div class="form-check form-check-inline d-inline-block me-3">
                                <input type="radio" id="customRadio4" name="type"
                                    value="{{ App\Enums\GroupType::SPECIAL->value }}"
                                    {{ $group->type == App\Enums\GroupType::SPECIAL->value ? 'checked' : '' }}
                                    class="form-check-input">
                                <label class="form-check-label" for="customRadio4">نوع گروه سهامی خاص</label>
                            </div>
                            <div class="form-check form-check-inline d-inline-block me-3">
                                <label class="form-check-label" for="customRadio4">
                                    تعداد کل سهام گروه : <p id="total_stock_display" class="d-inline-block mb-0">{{ old('normal_stock_count', $group->normal_stock_count) + old('prefered_stock_count', $group->prefered_stock_count) }}</p>
                                </label>
                            </div>
                        </div>
                        @error('type')
                        ...
                        <div class="d-flex gap-2 mt-3 {{ old('type', $group->type) == App\Enums\GroupType::SPECIAL->value ? '' : 'hidden' }}" id="shareInput">
                            <div class="flex-grow-1">
