            ...
            <form action="{{route('groups.update', $group->slug)}}" method="post" enctype="multipart/form-data">
                    ...
                    <div class="row">
                        <div>
                            <div class="mb-3">
                                ...
                                <input type="text" class="form-control" name="title" value="{{ old('title', $group->title) }}" id="title" placeholder="عنوان را وارد کنید" required="">
                                @error('title')
                        ...
                        <div>
                            <div>
                                ...
                                <textarea class="form-control" name="description" id="description" rows="3" placeholder="درباره ی گروه بنویسید">{{ old('description', $group->description) }}</textarea>
                                @error('description')
                                <span class="text-danger font-weight-bold">{{$message}}</span>
                                @enderror
                            </div>
                ...
                <div class="card-footer">
                    <div class="text-end mb-3">
                        <button type="submit" class="btn btn-primary">ویرایش</ذ>
                    </div>
                </div>
            </form>
        </div>
