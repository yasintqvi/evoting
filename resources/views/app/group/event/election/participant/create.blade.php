...
@section('content')
    <div class="page-title-head d-flex align-items-sm-center flex-sm-row flex-column gap-2">
        ...
        <div class="text-end">
            <ol class="breadcrumb m-0 py-0">
                ...
                <li class="breadcrumb-item active">تعیین گروه کنندگان</li>
            </ol>
        </div>
    </div>
            ...
            <div class="card">
                ...
                <div class="card-body">
                    <div class="tab-content">
                        @can(\App\Enums\Permission::CREATE_PARTICIPANTS->value)
                            <div class="tab-pane active show" id="description" role="tabpanel">
                                <form class="col-lg-12" action="{{ route('participants.store', [$group, $election]) }}"
                                    method="post">
                                    @csrf
                                    <div>
                        ...
                        @endcan
                        @if (in_array($election->type, [App\Enums\ElectionType::PRIVATE_JOINT, App\Enums\ElectionType::PRIVATE_JOINT_WITH_88]))
                            @can(\App\Enums\Permission::STORE_TABLE_PARTICIPANT->value)
                                <div class="tab-pane" id="review" role="tabpanel">
                                    <div class="row ">
                                        <form action="{{ route('participants.store-table-participen', [$group, $election]) }}"
    ...
    @if (in_array($election->type, [App\Enums\ElectionType::PRIVATE_JOINT, App\Enums\ElectionType::PRIVATE_JOINT_WITH_88]))
        @can(\App\Enums\Permission::IMPORT_PARTICIPANTS->value)
            <form class="card col-lg-5" action="{{ route('participants.import', [$group, $election]) }}" method="POST"
                ...
                <div>
                    <div class="card-header col-lg-12 border-bottom border-dashed d-flex align-items-center">
                        ...
                        <div class="col-lg-6 text-end">
                            <a href="{{ asset('assets/excel/نمونه اکسل گروه کنندگان.xlsx') }}" download=""
                                class="header-title" style="cursor: pointer">فایل نمونه اکسل</a>
                            <small class="text-muted d-block">کد ملی به عنوان رمز عبور تنظیم می‌شود</small>
                        </div>
                    </div>
