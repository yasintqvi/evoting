...
@php
    ...
    $currentCandidates = $election->candidates->where('candidate_type', App\Enums\CandidateType::DIRECTOR);
@endphp
                    ...
                    <div class="mb-3">
                        ...
                        @if ($isInspectorElection)
                            <select
                                ...
                                id="main_candidates"
                                multiple>
                                @foreach (collect(old('main_candidates_ids', $currentCandidates->pluck('user_id')->toArray()))->unique() as $selectedId)
                                    @php $selectedUser = $currentCandidates->firstWhere('user_id', $selectedId)?->user ?? \App\Models\User::find($selectedId); @endphp
                                    @if ($selectedUser)
                        ...
                        @else
                            <select
                                ...
                                <option
                                    value="{{ $user->id }}"
                                    {{ collect(old('main_candidates_ids', $currentCandidates->pluck('user_id')->toArray()))->contains($user->id) ? 'selected' : '' }}>
                                    {{ $user->fullName }}
                                </option>
                                @endforeach
                            </select>
